...
def frame(text, char, number_of_spaces=1, center=False):
    number_of_spaces = ceil(abs(number_of_spaces))  # making sure that number is both positive and not int
    longest_word = len(max(text, key=len))  # finding length of the longest word
    floor = (longest_word + 2 + (2 * number_of_spaces)) * char  # building floor based on len of longest word and character each side
    if center:
        wall = [
